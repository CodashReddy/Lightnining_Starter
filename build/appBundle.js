 ...
 * CLI version: 2.2.0
 *
 * Generated: Tue, 19 Jan 2021 12:32:15 GMT
 */
...
var APP_com_metrological_app_myawesomeapp = (function () {
  ...
  class Main extends Lightning.Component {
    static _template() {
      return {
        ...
        ,
        List: { type: MovieList, x: 300,y:200,  flex: { direction: "row", padding: 20, wrap:true },
        w: 1200}
      };
    }
  ...
  class PageDetails extends Lightning.Component {
    ...
    _init() {
      this._setState('notPlaying');
    }

    startTransitions() {
      this.tag("Overview").patch({ x: 700 });
      this.tag("Date").patch({ x: 700 });
      this.tag("Title").patch({ x: 700 });
      this.tag("Button").patch({ x: 700 });

      this.tag("Overview").setSmooth("x", 600);
      this.tag("Date").setSmooth("x", 600);
      this.tag("Title").setSmooth("x", 600);
      this.tag("Button").setSmooth("x", 600);
  ...
  async function _searchMovies(searchTerm) {
    console.log('search term.........>>>',searchTerm);
    const movieResults = await _fetchMoviesData();
    const tvResults = await _fetchTVData();
    let movieSearchResults = movieResults.results.filter(item => {
      return item.title.toLowerCase().includes(searchTerm.toLowerCase());
    });
    console.log("movie results....", movieSearchResults);
    let tvSearchResults = tvResults.results.filter(item => {
      return item.name.toLowerCase().includes(searchTerm.toLowerCase());
    });
    console.log("tv results", tvSearchResults);
    const searchResults = [];
    if(tvSearchResults.length!==0){
      searchResults.push(tvSearchResults);
    }
    if(movieSearchResults.length!==0){
      searchResults.push(movieSearchResults);
    }
  ...
  class Search extends Lightning.Component {
    ...
    set result(v) {
      ...
      if (this._searchRes.length === 0) ;else {
